...
It runs **automatically as a background task on store startup** (`with_semantic_search` spawns it; it never blocks startup, and on failure it logs an error so an operator can re-run it). It is also exposed on demand at `POST /admin/indices/vector/reconcile`.
