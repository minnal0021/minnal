...
/// - **payload\[1..\]** — [`chunk_document`](crate::semantic_search::chunking::chunk_document)'s N
...
mod tests {
    ...
    fn test_quantised_dot_product_accuracy() {
        use crate::semantic_search::cluster::{ClusterIndex, find_closest_cluster_id};
        use crate::semantic_search::index::distance_estimator::MultiBitQuanDotProductEstimator;
        use crate::semantic_search::quantisation::rabitq;
        ...
        let doc_path = concat!(env!("CARGO_MANIFEST_DIR"), "/src/test_data/doc_embedding.json");
        let query_path = concat!(env!("CARGO_MANIFEST_DIR"), "/src/test_data/query_embedding.json");
    ...
    fn test_multi_embedding_quantisation_produces_n_vector_indexes() {
        ...
        let vector_indexes: Vec<crate::semantic_search::index::vector_index::VectorIndex> = embeddings
            .iter()
    ...
    fn test_dual_quantisation_produces_both_styles() {
        ...
        let combined: Vec<crate::semantic_search::index::vector_index::VectorIndex> = std::iter::once(mb_vi).chain(sb_vis).collect();
    ...
    async fn test_search_returns_empty_with_no_query_embeddings() {
        let config = SemanticSearchConfig::default();
        let cluster_index = crate::semantic_search::cluster::ClusterIndex::from_clusters(Default::default());
        let results = search(
            &config,
            "test_ns",
            &cluster_index,
    ...
    fn make_cluster_index(entries: &[(u32, [f32; 4])]) -> crate::semantic_search::cluster::ClusterIndex {
    ...
    async fn test_search_skips_corrupt_entries_and_returns_valid_docs() {
        ...
        let before = crate::semantic_search::metrics::snapshot(ns);
        let results = search(
            &config,
            ns,
            &cluster_index,
