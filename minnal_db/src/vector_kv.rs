...
impl DbVectorStore {
    pub async fn new(db: &AsyncDb, namespace: &str) -> Result<Self, crate::KVError> {
        let sparse_ns = db.namespace(sparse_vectors_ns(namespace)).await?;
        let dense_ns = db.namespace(dense_vectors_ns(namespace)).await?;
...
pub async fn remove_queue_entry(db: &AsyncDb, namespace: &str, doc_id_bytes: &[u8]) -> Result<(), crate::KVError> {
    let queue_ns = db.namespace(PENDING_VEC_INDEX_NS.to_string()).await?;
...
///
/// [`VecIndexWorker`]: crate::doc_store::vec_index_worker::VecIndexWorker
pub async fn list_queue_entries(db: &AsyncDb) -> Result<Vec<QueueEntry>, crate::KVError> {
    let queue_ns = db.namespace(PENDING_VEC_INDEX_NS.to_string()).await?;
...
pub async fn get_queue_entry(db: &AsyncDb, namespace: &str, doc_id_bytes: &[u8]) -> Result<Option<QueueEntry>, crate::KVError> {
    let queue_ns = db.namespace(PENDING_VEC_INDEX_NS.to_string()).await?;
...
mod vector_upsert_tests {
    use super::*;
    use crate::AsyncDb;
    use crate::semantic_search::index::distance_estimator::{DotProductEstimatorState, MultiBitQuanDotProductEstimator};
    use crate::semantic_search::index::vector_index::{QuantisationStyle, VectorKvStore, score_rkyv_bytes};
...
mod queue_tests {
    ...
    async fn open_db(dir: &TempDir) -> AsyncDb {
        ...
        AsyncDb::open_with_config(path, crate::support::test_db_config()).await.unwrap()
    }
    ...
    async fn real_kv_search_profile() {
        ...
        for &cpd in &[1usize, 4, 8] {
            ...
            for &n_probes in &N_PROBES_SWEEP {
                ...
                eprintln!(
                    "     scoring: MaxSim+rerank {other:>9.1} µs  ({:>4.1}%)  [total − above]",
                    100.0 * other / tot
                );
...
mod real_recall {
    ...
    async fn real_recall_vs_nprobes() {
        ...
        if texts.len() < n_docs + n_queries {
            eprintln!(
                "SKIP real_recall_vs_nprobes: corpus has {} usable records, need {} docs + {} queries",
                texts.len(),
                n_docs,
                n_queries
            );
        ...
        assert_eq!(
            NPROBES[0], n_clusters,
            "NPROBES[0] must equal the cluster count ({n_clusters}) to be exhaustive ground truth"
        );
        let cluster_map: HashMap<u32, Cluster> = raw.into_iter().map(|(id, c)| (id, Cluster::new(id, c))).collect();
        ...
        if ok == 0 {
            eprintln!(
                "SKIP real_recall_vs_nprobes: every document embed failed — is the service up at {embed_url}?  first error: {:?}",
                first_err
            );
            db.shutdown().await.unwrap();
            return;
        ...
        async fn run_all(
            ...
            index: &ClusterIndex,
            store: &DbVectorStore,
            q_embs: &[crate::semantic_search::service::QueryEmbeddings],
