...
**The index checkpoint interval is the crash-replay window.** `scheduled_tasks.index_checkpoint_interval_ms` (default **1750 ms**) sets how often every field index is made durable, and after a crash each field replays the WAL from its last checkpoint. It was hardcoded at 15 minutes, which meant a crash could face fifteen minutes of writes to replay — measured at 7–27 minutes to restart a 16k-document store. It also caps WAL retention, since the index-replay watermark holds segments until a checkpoint advances. The per-tick cost is a flush per field index plus one marker write (tmp + rename + two fsyncs) per field.

**WAL replay batches by value — do not put it back to a per-key loop.** `activate_field_index` resolves every affected key first, groups the rows by extracted value, clears them all with `remove_all_for_rows`, then writes each value's bitmap once with `insert_many`. The blob store is append-only and both `insert` and `remove_all_for_row` re-serialise a whole bitmap, so the per-key form appends one dead copy of the bitmap *per key* — the dominant cost for a low-cardinality field. Measured: a 6,000-key replay went 626 ms → 104 ms, and cost is now *sublinear* in the window (more keys amortise over the same bucket writes). Replay is the one caller that can do this, because it knows its complete key set before it writes anything.
...
**`Wal` owns its file handle through a `crossbeam_epoch::Atomic`, which does NOT drop its pointee — every install needs an explicit reclaim.** Replaced handles go to `Guard::defer_destroy` (`rotate_to_segment`, `swap_file`); the handle still installed when the `Wal` itself dies is reclaimed by `impl Drop for Wal`, which drops it **immediately** rather than deferring. Both halves are load-bearing. Without the `Drop` impl, every dropped `Wal` leaked its boxed handle *and the open file descriptor* — invisible in normal use (one `Wal` per `Database`) but fatal to anything cycling WALs: `bench_wal`'s unsynced-append case climbed past 450,000 open fds in ten seconds and died on `EMFILE`, which for two reports running was misread as a benchmark-harness quirk. And deferring instead of dropping in place does not fix it: `defer_destroy` only runs once the global epoch advances, so a tight open/drop loop just piles the same descriptors up as pending garbage. Immediate reclamation is sound because `Drop::drop` takes `&mut self`, so no `Shared` or `&WalHandle` borrowed from it can still be live. Regression test: `dropping_a_wal_closes_its_file_descriptor` (`wal.rs`), which counts descriptors resolving into its own temp dir via `/proc/self/fd` — a process-wide count is useless here because cargo runs the module's tests concurrently in one process.
...
- **Liveness via the checkpoint trigger.** When segments are pinned, GC fires `IndexCheckpointTrigger::request()` (uncapped — `request_if_over_cap` returns early when the backpressure valve is disabled) so retention tracks checkpoint latency, not the periodic timer.
