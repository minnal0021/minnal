...
| `minnal_doc_store_api/` | Axum HTTP server; reads TOML config on startup; serves a unified `/stores` route tree for both doc and KV stores (kind selected by `store_type` in the create/import payload, resolved from the schema thereafter; KV data under `/stores/{ns}/kv`) |
