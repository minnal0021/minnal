...
- **Final ranking** — `[semantic_search.ranking]` (per-request overridable) fuses the two: `dense` (default), `sparse`, `rrf` (reciprocal rank fusion), or `zscore`, over the whole candidate set. Results carry `dense_score`, `sparse_score`, `fused_score`, and both ranks. Evaluate ranking changes with the BEIR harness (`semantic_search/beir_eval.rs`, data via `service/scripts/fetch_beir.sh`).

Requires an **external embedding service** (default: `http://localhost:8001`). Vector search will not work without it. Cluster centroids are bundled per model at `service/embedding_support/{gemma,qwen}/clusters.json` (256 centroids × 768 dims each); set `semantic_search.cluster_path` to the set matching the model the service serves. The key config knobs are `number_of_bits_for_dense_quantisation`, `n_probes`, `first_pass_sparse_search_top_k`, `window_size`, `sliding_size`, and `ranking` (final-order fusion). See `minnal_db/src/semantic_search/Semantic-Search-Architecture.md` for the full design.
