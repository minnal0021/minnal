...
- Writes are single-op only (`put`/`delete`/`merge`, each its own WAL-fsynced transaction). `merge(key, operand, closure)` is a read-modify-write made indivisible by a striped per-key lock that sits above the WAL lock — a **single-key** atomicity guarantee, not a transaction; its WAL record is the resolved `Upsert`/`Delete`, so replay is unchanged. There is no multi-op batch/transaction primitive — secondary structures (field index, vector index) are derived and reconstructable, so cross-namespace writes are ordered single ops (e.g. doc-store writes the document, then enqueues the embed marker), reconciled on recovery/re-index rather than committed atomically. **Exception:** the quantised vector-index payloads (`{ns}_sparse_vector`, `{ns}_sparse_vector_meta`, `{ns}_dense_vector`) are written **no-WAL** (`put_no_wal`) since they are bulky and re-embeddable — a crash before flush drops them and reconciliation re-enqueues the affected docs. Their cleanup **deletes** stay WAL-backed (a lost delete would orphan an index entry, which reconciliation cannot heal). The system-wide query-embedding cache (`system_qemb_cache`) populates no-WAL (`put_no_wal`) since it is TTL-bounded and fully regenerable, but the admin clear (`DELETE /admin/indices/vector/query-cache`) deletes **WAL-backed** — cached entries are periodically flushed to disk, so a lost clear tombstone would resurrect cleared entries after a crash. See `minnal_db/src/semantic_search/CLAUDE.md` → *Durability split*.
...
Key knobs: `sharding.num_buckets` (cannot change after data exists), `value_log.segment_size_bytes` (value-log segment size, default 256 MiB — GC's unit of work; NOT fixed at creation), `thresholds.page_gc_threshold` (which segments GC rewrites; keep below the bucket trigger), `sync.records_per_sync` (value-log fsync cadence — the WAL is fsynced on every write regardless), `thresholds.value_log_waste_threshold` (GC trigger), `thresholds.index_blob_waste_threshold` (field-index bitmap compaction trigger at checkpoint), `thresholds.max_pinned_wal_segments` (backstop on WAL held by the index-replay watermark, default 32; exceeding it knowingly strands part of a field index), `recovery.fail_log_dir` (where fail-log JSON files are written; defaults to `<db_path>/fail_logs`).

## Semantic search

`semantic_search` uses a **two-pass ANN search** with dual RaBitQ quantisation:
- **Pass 1 (sparse)** — 1-bit **document** chunk embeddings (sentence sliding windows) stored in `{ns}_sparse_vector`, prefix-scanned by IVF cluster for fast candidate selection. Scored with **ColBERT MaxSim** `S(q,d) = Σ_i max_j ⟨q_i, d_j⟩`. The query is **not chunked**: one whole-query embedding is both the Pass-1 vector and Pass 2's dense vector, so in production `S(q,d) = max_j ⟨q, d_j⟩` (best-matching chunk). Chunked queries were measured worse and far costlier (`minnal_db/src/semantic_search/query-embedding-report.md`).
- **Pass 2 (dense)** — multi-bit whole-doc embeddings stored in `{ns}_dense_vector`, fetched by `doc_id` for high-precision re-ranking.

Requires an **external embedding service** (default: `http://localhost:8001`). Vector search will not work without it. Cluster centroids are bundled per model at `service/embedding_support/{gemma,qwen}/clusters.json` (256 centroids × 768 dims each); set `semantic_search.cluster_path` to the set matching the model the service serves. The key config knobs are `number_of_bits_for_dense_quantisation`, `n_probes`, `first_pass_sparse_search_top_k`, and the document chunking `window_size` / `sliding_size` (changing those requires a re-index). See `minnal_db/src/semantic_search/Semantic-Search-Architecture.md` for the full design.
