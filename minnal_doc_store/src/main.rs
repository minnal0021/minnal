...
async fn demo_drop_index(store: &DocStore) -> Result<()> {
    ...
    let before = minnal_doc_store::DocStoreSchema::load(std::path::Path::new("/tmp/doc_store_demo/schemas"), "users").unwrap();
    log::info!(
    ...
    let after = minnal_doc_store::DocStoreSchema::load(std::path::Path::new("/tmp/doc_store_demo/schemas"), "users").unwrap();
    log::info!(
...
async fn demo_amend_schema(store: &DocStore) -> Result<()> {
    ...
    let schema = minnal_doc_store::DocStoreSchema::load(std::path::Path::new("/tmp/doc_store_demo/schemas"), "users").unwrap();
    log::info!(
        "final attributes: {:?}",
...
async fn demo_semantic_search(store: &DocStore) -> Result<()> {
    ...
    for query in queries {
        ...
        for (rank, r) in top.iter().enumerate() {
            ...
            let key_type = minnal_doc_store::KeyType::U64;
            let doc_id = minnal_doc_store::DocId::from_bytes(&r.document_id, key_type).unwrap_or(DocId::U64(0));
            let title = store
                .get("jobs", doc_id)
                .await?
