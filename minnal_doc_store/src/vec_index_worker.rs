...
use crate::store::SemanticSearchContext;
...
///
/// [`DocStore::with_vector_index_config`]: crate::store::DocStore::with_vector_index_config
/// [`DocStore::with_semantic_search`]: crate::store::DocStore::with_semantic_search
#[derive(Debug, Clone)]
...
impl VecIndexWorker {
    ...
    async fn process_one(&self, entry: &QueueEntry) -> Result<(), DocStoreError> {
        let vector_indexes = semantic_search::service::embed_document(&self.ctx.config, &self.ctx.cluster_index, &entry.text)
            .await
